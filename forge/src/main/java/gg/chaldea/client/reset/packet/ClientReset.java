    ...
    public static boolean handleClear(NetworkEvent.Context context) {
        CompletableFuture<Void> future = context.enqueueWork(() -> {
            ...
            if (Minecraft.getInstance().level == null) {
                // Ensure the GameData is reverted in case the client is reset during the
                // handshake.
            ...
            Minecraft.getInstance()
                    .clearLevel(new GenericDirtMessageScreen(new TranslatableComponent("connect.negotiating")));
            try {
        ...
        try {
            ...
            HandshakeHandler.LOGGER.error(HandshakeHandler.FMLHSMARKER, "Failed to clear, closing connection",
                    ex);
